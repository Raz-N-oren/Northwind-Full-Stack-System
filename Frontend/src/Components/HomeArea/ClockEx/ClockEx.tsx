...
class ClockEx extends Component<ClockExProps, ClockExState> { // Also Props and State
    ...
    private getTimeBy24hFormat(): string {
        const now = new Date();
        const time = now.toLocaleTimeString('en-US', { hour12: this.props.format === "24h" });
        return time;
    }
    ...
    private showTime1(): void {
        notifyService.success(this.getTime());
    }

    private showTime2 = () => {
        notifyService.success(this.getTimeBy24hFormat());
    }
